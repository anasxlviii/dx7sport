...
export default function ArticleEditPage() {
  ...
  function setAsFeatured(url: string) {
    if (article) setArticle({ ...article, featuredImage: url });
  }
          ...
          <div className="space-y-6">
            ...
            <div className="dxt-card bg-dark-surface overflow-hidden">
              <div className="p-5 border-b border-border-subtle flex items-center justify-between">
                ...
                <span className="text-[10px] text-gray-600 font-bold">يمكنك لصق صورة (Ctrl+V)</span>
              </div>
              ...
              <div className="p-5">
                {/* Gallery Tab - show added images */}
                {activeTab === 'gallery' && (
                  <div>
                    {/* Featured Image */}
                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">الصورة البارزة (الغلاف)</p>
                    {article.featuredImage ? (
                      <div className="relative mb-4 group">
                        <img src={article.featuredImage} alt="" className="w-full h-36 object-cover border border-lime/30" onError={e => (e.currentTarget.style.display='none')} />
                        <button onClick={() => setArticle({ ...article, featuredImage: null })}
                          className="absolute top-2 left-2 bg-black/80 text-red-400 text-[10px] font-bold px-2 py-1 border border-red-800 hover:bg-red-900/30 transition-all">
                          ✕ حذف
                        </button>
                      </div>
                    ) : (
                      <div className="w-full h-24 border border-dashed border-border-subtle flex items-center justify-center mb-4">
                        <p className="text-gray-700 text-[10px] font-bold uppercase">لا توجد صورة غلاف</p>
                      </div>
                    )}
                    ...
                    {images.length > 0 && (
                      <>
                        <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">صور إضافية ({images.length})</p>
                        <div className="grid grid-cols-2 gap-2">
                          {images.map(img => (
                            <div key={img.id} className="relative group">
                              <img src={img.url} alt={img.alt} className="w-full h-24 object-cover border border-border-subtle" onError={e => (e.currentTarget.style.display='none')} />
                              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1 items-center justify-center p-1">
                                <button onClick={() => setAsFeatured(img.url)}
                                  className="w-full text-[9px] font-bold py-1 bg-lime text-black uppercase hover:bg-lime/90">
                                  غلاف
                                </button>
                                <button onClick={() => deleteImage(img.id)}
                                  className="w-full text-[9px] font-bold py-1 bg-red-900/80 text-red-300 uppercase hover:bg-red-800">
                                  حذف
                                </button>
                              </div>
                            </div>
                    ...
                    {images.length === 0 && !article.featuredImage && (
                      <p className="text-center text-gray-700 text-[10px] py-4">ابحث عن صور أو ارفعها من التبويبات أعلاه</p>
                    )}
                  </div>
                )}
                ...
                {activeTab === 'search' && (
                  <div>
                    <div className="flex gap-2 mb-4">
                      <input type="text" placeholder="ابحث بالإنجليزية... e.g. Mbappe" value={imgSearchQ}
                        onChange={e => setImgSearchQ(e.target.value)} onKeyDown={e => e.key === 'Enter' && searchImages()}
                      ...
                      <button onClick={searchImages} disabled={searching}
                        className="px-3 py-2 bg-lime text-black text-[10px] font-black uppercase hover:bg-lime/90 disabled:opacity-50">
                        {searching ? '...' : 'بحث'}
                      </button>
                    </div>
                    {searching && <p className="text-center text-gray-500 text-xs py-4">جاري البحث...</p>}
                      ...
                      <div className="grid grid-cols-3 gap-1.5 max-h-80 overflow-y-auto">
                        {searchResults.map((r, i) => (
                          <div key={i} className="relative group cursor-pointer" onClick={() => addImageByUrl(r.url, r.title)}>
                            <img src={r.thumbnail || r.url} alt={r.title} className="w-full h-20 object-cover border border-border-subtle hover:border-lime transition-colors" onError={e => (e.currentTarget.style.display='none')} />
                            <div className="absolute inset-0 bg-lime/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                              <span className="text-white text-[9px] font-black bg-black/80 px-2 py-1">+ أضف</span>
                            </div>
                          </div>
                        ))}
                      </div>
                    )}
                    {!searching && searchResults.length === 0 && imgSearchQ && (
                      <p className="text-center text-gray-600 text-xs py-4">لا نتائج. جرب كلمات مختلفة.</p>
                    )}
                  </div>
                )}
                ...
                {activeTab === 'upload' && (
                  <div>
                    ...
                    <button onClick={() => fileInputRef.current?.click()} disabled={imgLoading}
                      className="w-full py-8 border-2 border-dashed border-border-subtle hover:border-lime text-gray-500 hover:text-lime text-xs font-bold uppercase tracking-widest transition-all disabled:opacity-50 flex flex-col items-center gap-3">
                      <span className="text-3xl">{imgLoading ? '⏳' : '⬆'}</span>
                      <span>{imgLoading ? 'جاري الرفع...' : 'اختر صور أو اسحبها هنا'}</span>
                      <span className="text-[10px] text-gray-700 normal-case">أو الصق صورة من الحافظة (Ctrl+V) في أي مكان</span>
                    </button>
                    <p className="text-center text-gray-700 text-[10px] mt-3">JPG, PNG, WebP — حتى 8MB لكل صورة</p>
                  </div>
                )}
                ...
                {activeTab === 'paste' && (
                  <div className="space-y-3">
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-3">الصق رابط الصورة مباشرة</p>
                    <textarea placeholder="https://example.com/image.jpg&#10;https://example.com/image2.jpg&#10;(رابط واحد في كل سطر)"
                      value={pasteUrl} onChange={e => setPasteUrl(e.target.value)}
                      rows={5} className="w-full px-3 py-2 bg-black border border-border-subtle text-white text-xs focus:border-lime focus:outline-none" dir="ltr" />
                    <button onClick={async () => {
                        const urls = pasteUrl.split('\n').map(u => u.trim()).filter(Boolean);
                        for (const url of urls) await addImageByUrl(url);
                      }} disabled={imgLoading || !pasteUrl.trim()}
                      className="w-full py-2 bg-lime text-black text-[10px] font-black uppercase tracking-widest hover:bg-lime/90 disabled:opacity-50 transition-all">
                      {imgLoading ? 'جاري الإضافة...' : `إضافة ${pasteUrl.split('\n').filter(u=>u.trim()).length} صورة`}
                    </button>
                  </div>
                )}
              </div>
            ...
            {metadata.factBox && (
              <div className="dxt-card p-6 bg-dark-surface">
                <h3 className="text-xs font-black text-lime uppercase tracking-[0.2em] mb-4">معلومات تكتيكية</h3>
                <div className="text-sm text-gray-400 font-medium whitespace-pre-line leading-relaxed" dir="auto">{metadata.factBox}</div>
              </div>
            )}
            ...
            {article.sources?.length > 0 && (
              <div className="dxt-card p-6 bg-dark-surface">
                <h3 className="text-xs font-black text-lime uppercase tracking-[0.2em] mb-4">المصادر ({article.sources.length})</h3>
                <div className="space-y-3">
                  {article.sources.map(source => (
                    <div key={source.id} className="text-sm border-b border-border-subtle pb-3 last:border-0">
                      <a href={source.url} target="_blank" rel="noopener" className="text-gray-300 hover:text-lime line-clamp-2 font-bold transition-colors" dir="ltr">{source.title}</a>
                      <span className={`text-[10px] mt-1 inline-block px-2 py-0.5 border font-bold uppercase ${source.credibility === 'high' ? 'border-lime text-lime' : 'border-gray-700 text-gray-600'}`}>
            ...
            <div className="dxt-card p-6 bg-dark-surface">
              ...
              <div className="space-y-3 text-sm text-gray-400">
                ...
                <div className="flex justify-between items-center border-b border-border-subtle pb-2">
                  <span className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">تاريخ الإنشاء</span>
                  <span className="font-mono text-xs">{new Date(article.createdAt).toLocaleDateString('ar-EG')}</span>
                </div>
                {article.publishedAt && (
                  <div className="flex justify-between items-center border-b border-border-subtle pb-2">
                    <span className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">تاريخ النشر</span>
                    <span className="font-mono text-xs text-lime">{new Date(article.publishedAt).toLocaleDateString('ar-EG')}</span>
                  </div>
                )}
                <div>
            ...
            {article.sourcePostUrl && (
              <div className="dxt-card p-6 bg-dark-surface">
                <h3 className="text-xs font-black text-lime uppercase tracking-[0.2em] mb-3">منشور المصدر</h3>
                <a href={article.sourcePostUrl} target="_blank" rel="noopener" className="text-xs text-gray-500 hover:text-lime break-all transition-colors font-mono" dir="ltr">{article.sourcePostUrl}</a>
              </div>
            )}
          </div>
        </div>
