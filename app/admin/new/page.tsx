...
export default function NewArticlePage() {
  ...
  // Clipboard Paste Handler (for AI mode)
  useEffect(() => {
  ...
  async function saveManual() {
    ...
    try {
      const res = await fetch('/api/articles', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: title.trim(), content: content.trim(), excerpt: excerpt.trim(), category, status: 'draft' }),
      });
      const data = await res.json();
      if (res.ok) router.push(`/admin/article/${data.article.id}`);
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="dxt-card p-8 bg-dark-surface">
              ...
              <textarea value={content} onChange={e => setContent(e.target.value)} placeholder="اكتب المقال هنا... استخدم Markdown للتنسيق (**عريض**, ## عناوين, - قوائم)" dir="auto"
                rows={24} className="w-full px-4 py-3 bg-black border border-border-subtle text-gray-300 font-mono text-sm focus:border-lime focus:outline-none transition-colors" />
            </div>
          </div>
          <div className="space-y-6">
            <div className="dxt-card p-6 bg-dark-surface">
              ...
              <div className="space-y-4">
                ...
                <button onClick={saveManual} disabled={saving || !title.trim() || !content.trim()}
            ...
            <div className="dxt-card p-6 bg-dark-surface border-dashed border-zinc-800">
              <div className="flex flex-col items-center text-center py-6 opacity-50">
                <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">💡 نصيحة</p>
                <p className="text-xs text-gray-600 mt-2 leading-relaxed">يمكنك استخدام Markdown لتنسيق المقال:<br />**نص عريض** — ## عنوان — - قائمة</p>
              </div>
            </div>
          </div>
        </div>
